...
def mock_read_file_safe():
    with patch('nexus_worker.tools.explain.read_file_safe', return_value=("file content", 10)) as mock:
        yield mock
...
class TestWorkerExplainCodeHappyPath:
    @pytest.mark.asyncio
    async def test_explain_code_success(self, mock_provider, mock_prompt_engine, mock_metrics, mock_call_tracker, mock_read_file_safe, mock_with_retry, mock_log_tool_call):
...
class TestWorkerExplainCodeEdgeCases:
    @pytest.mark.asyncio
    async def test_explain_code_empty_file(self, mock_provider, mock_prompt_engine, mock_metrics, mock_call_tracker, mock_with_retry, mock_log_tool_call):
    ...
    @pytest.mark.asyncio
    async def test_explain_code_large_file(self, mock_provider, mock_prompt_engine, mock_metrics, mock_call_tracker, mock_with_retry, mock_log_tool_call):
        large_content = "line\n" * 10000
        with patch('nexus_worker.tools.explain.read_file_safe', return_value=(large_content, 10000)):
            result = await worker_explain_code(
                file_path="large.py",
                provider=mock_provider,
                prompt_engine=mock_prompt_engine,
...
class TestWorkerExplainCodeErrorHandling:
    ...
    async def test_explain_code_file_not_found(self, mock_provider, mock_prompt_engine, mock_metrics, mock_call_tracker):
        with patch('nexus_worker.tools.explain.read_file_safe', side_effect=FileNotFoundError("File not found")):
            result = await worker_explain_code(
    ...
    async def test_explain_code_permission_error(self, mock_provider, mock_prompt_engine, mock_metrics, mock_call_tracker):
        with patch('nexus_worker.tools.explain.read_file_safe', side_effect=PermissionError("Permission denied")):
            result = await worker_explain_code(
    ...
    async def test_explain_code_worker_error(self, mock_provider, mock_prompt_engine, mock_metrics, mock_call_tracker, mock_read_file_safe):
        with patch('nexus_worker.tools.explain.with_retry', side_effect=WorkerError("Worker failed")):
            result = await worker_explain_code(
                file_path="test.py",
                provider=mock_provider,
                prompt_engine=mock_prompt_engine,
