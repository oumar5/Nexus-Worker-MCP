"""Outil MCP : worker_review_code

Lit un fichier et délègue la revue de code au Worker.
Retourne une revue structurée JSON sans encombrer le contexte du Cerveau.
"""
...
async def worker_review_code(
    ...
    """Effectue une revue de code en déléguant l'analyse au Worker.

    Args:
        file_path: Chemin du fichier à réviser.
        ...
        focus: Aspect spécifique à évaluer (ex: "security", "performance").
        cache: Cache de résultats optionnel.

    Returns:
        Résultat JSON stringifié avec la revue structurée ou l'erreur.
    """
    tool_name = "worker_review_code"
    ...
    system_prompt = prompt_engine.get_system_prompt("review")
    user_prompt = f"Fichier à réviser: {file_path} ({total_lines} lignes)\n\n"
    if focus:
        user_prompt += f"Focus demandé: {focus}\n\n"
    user_prompt += f"Code:\n{content}"
    ...
    try:
        ...
        # Tenter de parser le JSON retourné par le Worker
        try:
            review_data = json.loads(response.content)
        except json.JSONDecodeError:
            review_data = {"raw_review": response.content}
